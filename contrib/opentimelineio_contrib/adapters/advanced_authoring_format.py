...
def read_from_file(filepath, simplify=True, transcribe_log=False, clip_timecode_offset=True):
    """Reads AAF content from `filepath` and outputs a OTIO timeline object.

    Args:
        filepath (str): AAF filepath
        simplify (bool, optional): simplifies timeline structure by stripping empty items
        transcribe_log (bool, optional): log activity as items are getting transcribed
        clip_timecode_offset (bool, optional): if `True`, clip ranges will be offset by the global timecode
